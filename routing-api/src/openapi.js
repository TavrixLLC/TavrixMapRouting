export function buildOpenApiSpec({ port, regionalUrl, worldUrl, region, importantAreas }) {
  ...
  const directionsQuery = [
    ...
    { name: 'max_alternatives', schema: { type: 'integer', minimum: 1, maximum: 3, default: 1 } },
    ...
    { name: 'overview', schema: { type: 'string', enum: ['full', 'simplified', 'false'], default: 'full' } },
    { name: 'annotations', schema: { type: 'string', example: 'duration,distance,speed' }, description: 'Comma-separated annotations.' },
...
function schemas(areaExample) {
  ...
  const routeOptions = {
    ...
    max_alternatives: { type: 'integer', minimum: 1, maximum: 3 },
    ...
    overview: { type: 'string', enum: ['full', 'simplified', 'false'], default: 'full', description: 'Route overview geometry detail. Defaults to full.' },
    annotations: { oneOf: [{ type: 'string' }, { type: 'array', items: { type: 'string' } }] },
  ...
  return {
    ...
    Costing: { type: 'string', enum: ['auto', 'pedestrian', 'bicycle', 'motor_scooter', 'truck', 'bus', 'taxi'], default: 'auto' },
    ...
    RouteResponse: { type: 'object', required: ['distance', 'duration', 'units', 'geometry', 'routes', 'engine', 'area', 'warnings'], properties: { distance: { type: 'number' }, duration: { type: 'number' }, units: { type: 'string' }, geometry: { $ref: '#/components/schemas/Geometry' }, routes: { type: 'array', items: { $ref: '#/components/schemas/Route' } }, engine, area, warnings: warning, raw: { type: 'object', description: rawDescription } } },
    MatrixResponse: { type: 'object', required: ['sources_count', 'targets_count', 'cells', 'durations', 'distances', 'engine', 'area', 'warnings'], properties: { sources_count: { type: 'integer' }, targets_count: { type: 'integer' }, cells: { type: 'integer' }, durations: { type: 'array', items: { type: 'array', items: { type: 'number', nullable: true } } }, distances: { type: 'array', items: { type: 'array', items: { type: 'number', nullable: true } } }, engine, area, warnings: warning, raw: { type: 'object', description: rawDescription } } },
    IsochroneResponse: { type: 'object', required: ['type', 'features', 'engine', 'area', 'warnings'], additionalProperties: true, properties: { type: { type: 'string', enum: ['FeatureCollection'] }, features: { type: 'array', items: { type: 'object' } }, engine, area, warnings: warning } },
    MapMatchResponse: { type: 'object', required: ['confidence', 'matched_points', 'unmatched_points', 'snapped_distance_m', 'geometry', 'routes', 'engine', 'area', 'warnings'], properties: { confidence: { type: 'number', nullable: true }, matched_points: { type: 'integer' }, unmatched_points: { type: 'integer' }, snapped_distance_m: { type: 'number' }, geometry: { $ref: '#/components/schemas/Geometry' }, routes: { type: 'array', items: { $ref: '#/components/schemas/Route' } }, engine, area, warnings: warning, raw: { type: 'object', description: rawDescription } } },
    SnapResult: { type: 'object', required: ['input', 'snapped', 'distance_meters', 'edge_id', 'road_name', 'road_class', 'speed', 'maxspeed'], properties: { input: coordinate, snapped: coordinate, distance_meters: { type: 'number' }, edge_id: { type: 'string', nullable: true }, road_name: { type: 'string' }, road_class: { type: 'string', nullable: true }, speed: { type: 'number', nullable: true }, maxspeed: { type: 'number', nullable: true } } },
    SnapResponse: { type: 'object', required: ['results', 'engine', 'area', 'warnings'], properties: { results: { type: 'array', items: { $ref: '#/components/schemas/SnapResult' } }, engine, area, warnings: warning } },
    DistanceResponse: { type: 'object', required: ['haversine_distance_m', 'route_distance_m', 'duration_s', 'engine', 'area', 'warnings'], properties: { haversine_distance_m: { type: 'number' }, route_distance_m: { type: 'number' }, duration_s: { type: 'number' }, engine, area, warnings: warning } },
    OptimizationResponse: { type: 'object', required: ['ordered_jobs', 'route', 'total_distance_m', 'total_duration_s', 'optimizer', 'optimal', 'engine', 'area', 'warnings'], properties: { ordered_jobs: { type: 'array' }, route: { $ref: '#/components/schemas/RouteResponse' }, total_distance_m: { type: 'number' }, total_duration_s: { type: 'number' }, optimizer: { type: 'string', example: 'nearest_neighbor' }, optimal: { type: 'boolean', example: false }, engine, area, warnings: warning } },
    ...
    AreaHealthResponse: { type: 'object', required: ['area', 'ok', 'url', 'latency_ms', 'error'], properties: { area: { type: 'string' }, ok: { type: 'boolean' }, url: { type: 'string', nullable: true }, latency_ms: { type: 'number' }, error: { type: 'string', nullable: true }, status: { type: 'string' } } },
    AreaCoverageResponse: { type: 'object', required: ['area', 'bounds', 'graph_path', 'build_id', 'enabled_profiles', 'min_lat', 'max_lat', 'min_lon', 'max_lon'], properties: { area: { type: 'string' }, bounds: { $ref: '#/components/schemas/Bounds' }, graph_path: { type: 'string' }, build_id: { type: 'string', nullable: true }, enabled_profiles: { type: 'array', items: { type: 'string' } }, min_lat: { type: 'number' }, max_lat: { type: 'number' }, min_lon: { type: 'number' }, max_lon: { type: 'number' } } },
    ...
    HealthResponse: { type: 'object', required: ['status', 'service', 'mode', 'region', 'active_build', 'engines'], properties: { status: { type: 'string' }, service: { type: 'string' }, mode: { type: 'string' }, region: { type: 'string' }, active_build: { type: 'string', nullable: true }, engines: { type: 'object', required: ['regional', 'world'], properties: { regional: { allOf: [{ $ref: '#/components/schemas/DependencyStatus' }] }, world: { allOf: [{ $ref: '#/components/schemas/DependencyStatus' }] } } } } },
    LiveResponse: { type: 'object', required: ['status'], properties: { status: { type: 'string', example: 'ok' } } },
    ReadyResponse: { type: 'object', required: ['status', 'regional', 'active_graph_exists', 'active_build', 'areas_count'], properties: { status: { type: 'string' }, regional: { $ref: '#/components/schemas/DependencyStatus' }, active_graph_exists: { type: 'boolean' }, active_build: { type: 'string', nullable: true }, areas_count: { type: 'integer' } } },
    DependenciesResponse: { type: 'object', required: ['regional', 'world'], properties: { regional: { $ref: '#/components/schemas/DependencyStatus' }, world: { $ref: '#/components/schemas/DependencyStatus' } } },
    BuildsResponse: { type: 'object', required: ['builds'], properties: { builds: { type: 'array', items: { type: 'string' } } } },
    CurrentBuildResponse: { type: 'object', required: ['metadata'], properties: { metadata: { type: 'object', nullable: true } } },
